...
module.exports = [
    // Destination
    function (session) {
        session.send('Welcome to the Sharjah Museums finder!');
        builder.Prompts.text(session, 'Please enter your theam , sicence , islamic , cuclture');
    },
    function (session, results, next) {
        session.dialogData.destination = results.response;
        session.send('Looking for Museums with theam %s', results.response);
        next();
    },

    // Check-in
    function (session) {
        builder.Prompts.time(session, 'When do you plan to visit?');
    },
    function (session, results, next) {
    ...
    // Nights
    function (session) {
        builder.Prompts.number(session, 'How many persons?');
    },
    function (session, results, next) {
    ...
    function (session) {
        ...
        session.send(
            'Ok. Searching for Musumes in %s from %d/%d to %d/%d...',
            destination,
