...
var PuristConnector = (function () {
    function PuristConnector(settings) {
    ...
    PuristConnector.prototype.listen = function () {
        var _this = this;
        function defaultNext() { }
        // Emran transform message
              ...
              //  _this.verifyBotFramework(req, res, next || defaultNext);
                ...
                req.on('end', function () {
                    try {
                        ...
                        logger.error('PuristConnector: receive - invalid request data received.');
                        res.send(400);
                        res.end();
                        return;
                    }
                    ...
                    //  var vv = {"type":"message","text":req.body.body,
                    //             "textFormat":"plain","timestamp":Date.now().toISOString(),"localTimestamp":Date.now().toISOString(),
                    //             "textLocale":"en-US","sourceEvent":{"clientActivityId":"1518552584556.3204382615514274.6"},
                    //             "attachments":[],"entities":[],"address":{"id":"0dem2355idn06","purisit":"emulator",
                    //             "user":{"id":req.body.end_cusomer_id,"name":req.body.end_customer_name},
                    //             "conversation":{"id":req.body.conversation_id},"bot":{"id":"default-bot","name":"Bot"},
                    //             "serviceUrl":"http://api.puristchat.com"}
                    //             };
                     var msgWanted =  {
                        "type": "message",
                        "text": req.body.body,
                        "from": {
                          "id": req.body.end_cusomer_id,
                          "name": req.body.end_customer_name
                        },
                        "locale": "en-US",
                        "textFormat": "plain",
                        "timestamp": Date.now(),
                        "channelData": {
                          "clientActivityId": "1518725565660.5802817822552229.6"
                        },
                        "id": "451c1cimjh6h"
                      };               
                     _this.dispatch(msgWanted, res, next);
                    
                });
            }
        };
    };
    PuristConnector.prototype.verifyBotFramework = function (req, res, next) {
        var _this = this;
        var token;
        ...
        if (token) {
            ...
            if (isEmulator) {
                if ((decoded_1.payload.ver === '2.0' && decoded_1.payload.azp !== this.settings.appId) ||
                    ...
                    logger.error('PuristConnector: receive - invalid token. Requested by unexpected app ID.');
                    res.status(403);
                    res.end();
                    next();
                    return;
                ...
                if (key) {
                    try {
                        ...
                        if (typeof req.body.channelId !== 'undefined' &&
                            ...
                            logger.error("PuristConnector: receive - endorsements validation failure. " + errorDescription);
                        ...
                        if (typeof decoded_1.payload.serviceurl !== 'undefined' &&
                            ...
                            logger.error("PuristConnector: receive - serviceurl mismatch. " + errorDescription);
                            throw new Error(errorDescription);
                        }
                    }
                    catch (err) {
                        logger.error('PuristConnector: receive - invalid token. Check bot\'s app ID & Password.');
                        res.send(403, err);
                        res.end();
                        next();
                        return;
                    ...
                    logger.error('PuristConnector: receive - invalid signing key or OpenId metadata document.');
                    res.status(500);
                    res.end();
                    next();
                    return;
            ...
            logger.warn(req.body, 'PuristConnector: receive - emulator running without security enabled.');
            this.dispatch(req.body, res, next);
        }
        else {
            logger.error('PuristConnector: receive - no security token sent.');
            res.status(401);
            res.end();
            next();
        }
    };
    PuristConnector.prototype.onEvent = function (handler) {
        this.onEventHandler = handler;
    };
    PuristConnector.prototype.onInvoke = function (handler) {
        this.onInvokeHandler = handler;
    };
    PuristConnector.prototype.send = function (messages, done) {
        var _this = this;
        var addresses = [];
            ...
            try {
                if (msg.type == 'delay') {
                    ...
                    if (serviceUrlExists) {
                        ...
                        logger.error("PuristConnector: send - " + msg_1);
                        cb(new Error(msg_1));
                    }
                }
            }
    ...
    PuristConnector.prototype.startConversation = function (address, done) {
        if (address && address.user && address.bot && address.serviceUrl) {
            ...
            this.authenticatedRequest(options, function (err, response, body) {
                ...
                if (err) {
                    logger.error('PuristConnector: startConversation - error starting conversation.');
                }
                done(err, adr);
            });
        }
        else {
            logger.error('PuristConnector: startConversation - address is invalid.');
            done(new Error('Invalid address.'));
        }
    };
    PuristConnector.prototype.update = function (message, done) {
        ...
        if (message.address && address.serviceUrl) {
            ...
            logger.error('PuristConnector: updateMessage - message is missing address or serviceUrl.');
            done(new Error('Message missing address or serviceUrl.'), null);
        }
    };
    PuristConnector.prototype.delete = function (address, done) {
    ...
    PuristConnector.prototype.getData = function (context, callback) {
        var _this = this;
        try {
    ...
    PuristConnector.prototype.saveData = function (context, data, callback) {
        var _this = this;
        console.warn(StateApiDreprecatedMessage);
        var list = [];
    ...
    PuristConnector.prototype.onDispatchEvents = function (events, callback) {
    ...
    PuristConnector.prototype.dispatch = function (msg, res, next) {
        try {
            // Emran normalize here
            var v = {"type":"message","text":msg.text,"textFormat":"plain","timestamp":"2018-02-13T20:19:56.737Z","localTimestamp":"2018-02-14T00:19:56+04:00","textLocale":"en-US","sourceEvent":{"clientActivityId":"1518552584556.3204382615514274.6"},"attachments":[],"entities":[],"address":{"id":"0dem2355idn06","purisit":"emulator","user":{"id":"default-user","name":"User"},"conversation":{"id":"mn67682n4g9c"},"bot":{"id":"default-bot","name":"Bot"},"serviceUrl":"http://localhost:50017"}}

            this.prepIncomingMessage(v);
            logger.info(msg, 'PuristConnector: message received.');
            ...
            this.onDispatchEvents([msg], function (err, body, status) {
                if (err) {
                    ...
                    res.end();
                    next();
                    logger.error('PuristConnector: error dispatching event(s) - ', err.message || '');
                }
                else if (body) {
    ...
    PuristConnector.prototype.isInvoke = function (event) {
        return (event && event.type && event.type.toLowerCase() == consts.invokeType);
    };
    PuristConnector.prototype.postMessage = function (msg, lastMsg, cb, method) {
        if (method === void 0) { method = 'POST'; }
        // EMRAN
        logger.info(address, 'PuristConnector: sending message.');
        ...
        var token =  'bcdcbc8e34ddfd59e84e9fd171b6c292' ;
        var opt = {
            url:sendAPI,
            method: 'POST',               
            headers: {     
        ...
        // this.authenticatedRequest(opt, function (err, response, body) {
        //     if (!err) {
        //         if (body && body.id) {
    ...
    PuristConnector.prototype.authenticatedRequest = function (options, callback, refresh) {
        var _this = this;
    ...
    PuristConnector.prototype.tokenExpired = function () {
        return Date.now() >= this.accessTokenExpires;
    };
    PuristConnector.prototype.tokenHalfWayExpired = function (secondstoHalfWayExpire, secondsToExpire) {
    ...
    PuristConnector.prototype.refreshAccessToken = function (cb) {
        var _this = this;
    ...
    PuristConnector.prototype.getAccessToken = function (cb) {
        var _this = this;
    ...
    PuristConnector.prototype.addUserAgent = function (options) {
        if (!options.headers) {
            options.headers = {};
        }
        options.headers['User-Agent'] = USER_AGENT;
    };
    PuristConnector.prototype.addAccessToken = function (options, cb) {
    ...
    PuristConnector.prototype.getStoragePath = function (address) {
        var path;
        switch (address.channelId) {
            case 'emulator':
                if (address.serviceUrl) {
                    ...
                    throw new Error('PuristConnector.getStoragePath() missing address.serviceUrl.');
                }
                break;
            default:
    ...
    PuristConnector.prototype.prepIncomingMessage = function (msg) {
    ...
    PuristConnector.prototype.prepOutgoingMessage = function (msg) {
        if (msg.attachments) {
            var attachments = [];
    ...
    return PuristConnector;
}());
exports.PuristConnector = PuristConnector;
var toAddress = {
    'id': 'id',
    // 'channelId': 'channelId',
    'channelId': 'purisit',
